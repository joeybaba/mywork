...
_DXY_DATA_FILE_ = '/Users/joey/Documents/PycharmProjects/DXY-COVID-19-Data/csv/DXYArea.csv'
...
def set_font(font_file):
    if not os.path.exists(font_file):
        print(
            font_file + " not found.  If you wish to display Chinese characters in plots, please use set_font() to set the path to the font file.")
    else:
...
def load_chinese_raw():
    ...
    data['update_date'] = data[
        'update_time'].dt.date  # add date for daily aggregation, if without to_datetime, it would be a dateInt object, difficult to use
    # display basic info
...
def rename_cities(data):
    '''
    Sometimes, for example 2/3/2020, on some time data, the CSV data contains city_name entries such as "南阳", "商丘", but at other time data, it contains "南阳（含邓州）",  and "商丘（含永城）", etc.  They should be treated as the same city
    ...
    '''
    dup_frm = data[np.logical_and(np.logical_not(data['city_name'].isnull()), data['city_name'].str.contains('（'))]
    rename_dict = {}
    if len(dup_frm) >= 0:
...
def aggDaily(df):
    ...
    frm_list = []
    drop_cols = ['province_' + field for field in
                 ['confirmed', 'suspected', 'cured', 'dead']]  # these can be computed later
    drop_cols += ['provinceEnglishName', 'cityEnglishName', 'province_zipCode']
    for key, frm in df.drop(columns=drop_cols).sort_values(['update_date']).groupby(
            ['province_name', 'city_name', 'update_date']):
    ...
    out = out.drop(columns=[
        'cum_suspected'])  # the suspected column from csv is not reliable, may keep this when the upstream problem is solved
...
def add_daily_new(df, group_keys=['province_name', 'city_name'], diff_cols=['cum_confirmed', 'cum_dead', 'cum_cured'],
...
def tsplot_conf_dead_cured(df, figsize=(13, 10), fontsize=18, logy=False, title=None):
    ...
    plot_df.plot(y=['cum_confirmed'], style='-*', ax=ax1, grid=True, figsize=figsize, logy=logy, color='black',
                 marker='o')
...
def cross_sectional_bar(df, date_str, col, groupby='province_name', largestN=0, figsize=(13, 10), fontsize=15,
    ...
    if title is not None:
        ax.set_title(title, fontproperties=_FONT_PROP_,
                     fontsize=fontsize)  # because there is only one axes, so setting title on ax level, rather than the "suptitle" in figure level looks better
    return fig
...
def add_moving_average(df, group_col, win_size):
    ma = df.reset_index(group_col).groupby(group_col)['new_confirmed', 'new_dead'].rolling(win_size,
                                                                                           min_period=1).mean()
...
def IL_death_demographic_early():
    ...
    ages.append(60)
    references.append(
        'http://www.dph.illinois.gov/news/public-health-officials-announce-first-illinois-coronavirus-disease-death')
    ...
    ages.append(40)
    references.append(
        'http://www.dph.illinois.gov/news/public-health-officials-announce-deaths-three-more-individuals-illinois-coronavirus-disease')
    ...
    ages.append(70)
    references.append(
        'http://www.dph.illinois.gov/news/public-health-officials-announce-163-new-cases-coronavirus-disease')
    ...
    ages.append(70)
    references.append(
        'http://www.dph.illinois.gov/news/public-health-officials-announce-168-new-cases-coronavirus-disease')
    ...
    ages.append(80)
    references.append(
        'http://www.dph.illinois.gov/news/public-health-officials-announce-296-new-cases-coronavirus-disease')
    ...
    ages.append(80)
    references.append(
        'http://www.dph.illinois.gov/news/public-health-officials-announce-236-new-cases-coronavirus-disease')
    ...
    ages.append(50)
    references.append(
        'http://www.dph.illinois.gov/news/public-health-officials-announce-250-new-cases-coronavirus-disease')
    ...
    ages.append(90)
    references.append(
        'http://www.dph.illinois.gov/news/public-health-officials-announce-330-new-cases-coronavirus-disease')
...
def parse_IL_death_demographic(date_range, max_page=5):
    ...
    early_demographic = early_demographic[
        early_demographic['Date'].between(date_range[0], date_range[-1], inclusive=True)]
    ...
    for date in dl:
        ...
        for elm in elm_list:
            ...
            detail_link = IDPH_BASE + elm.parent.find('span', {'class': 'field-content'}).a.get(
                'href')  # use the "read-more >>" link, otherwise the main page may be incomplete
